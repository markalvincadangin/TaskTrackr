...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($description) || empty($deadline) || empty($category_id)) {
        ...
        header("Location: ../public/projects.php");  // Redirect back to the projects page
        exit();
    }

    // Insert the new project into the database
    $query = "INSERT INTO Projects (title, description, deadline, category_id, created_by) VALUES (?, ?, ?, ?, ?)";
    ...
    $stmt->bind_param("ssssi", $title, $description, $deadline, $category_id, $user_id);
    $result = $stmt->execute();
